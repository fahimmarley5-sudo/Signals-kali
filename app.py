...
# Apply premium blended CSS layouts
st.markdown("""
    <style>
...
with st.expander("🛠️ System Configuration Dashboard", expanded=True):
    user_token = st.text_input("Paste your new Deriv API Token here:", type="password")
...
if not user_token:
    st.info("👋 Welcome! Please paste your new Deriv API Token into the field inside the 'System Configuration Dashboard' box above to activate the live matching tool.")
else:
    ...
    # Production WebSocket Engine Connected to Deriv Real-time Network
    signal_hub_html = """
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        <div class="hub-card">
            <div class="live-price-box" id="price-display">Authenticating Feed...</div>
        ...
        <script>
            ...
            let authToken = "REPLACE_AUTH_TOKEN";
            ...
            function initDataStream() {
                ...
                // CONNECTS VIA REGISTERED APP GATEWAYS TO PERMIT STREAMING DATA CODES
                ...
                ws.onmessage = (event) => {
                    ...
                    if (data.error) {
                        document.getElementById('price-display').innerHTML = "<span style='font-size:1.1rem; color:#f72585;'>Connection Authorization Failed</span>";
                    }
                };

                ws.onerror = () => {
                    document.getElementById('price-display').innerHTML = "<span style='font-size:1rem; color:#888;'>Data stream sync reset...</span>";
                };
